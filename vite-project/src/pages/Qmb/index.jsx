...
import { questionsQmb } from '../../components/Questions/indexQmbQuestions';
...
const Qmb = () => {
  ...
  const [progress, setProgress] = useState(() => {
    return parseInt(localStorage.getItem('progressQmb')) || 0;
  });
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [score, setScore] = useState(0);
  const [selectedOption, setSelectedOption] = useState("");
  const [questions, setQuestions] = useState(questionsQmb);
  const [incorrectQuestions, setIncorrectQuestions] = useState([]);

  useEffect(() => {
    const storedIncorrectQuestions = JSON.parse(localStorage.getItem('incorrectQuestionsQmb')) || [];
  ...
  const handleAnswer = (option) => {
    ...
    setTimeout(() => {
      ...
      if (currentQuestion < questions.length - 1) {
        ...
        // Quiz complete
        localStorage.setItem('progressQmb', Math.min((score + 1) * 10, 100));
        localStorage.setItem('incorrectQuestionsQmb', JSON.stringify(incorrectQuestions));
