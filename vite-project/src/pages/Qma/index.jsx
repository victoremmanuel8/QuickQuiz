...
import { questionsQma } from '../../components/Questions/indexQmaQuestions';
...
const Qma = () => {
  ...
  const [progress, setProgress] = useState(() => {
    return parseInt(localStorage.getItem('progressQma')) || 0;
  });
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [score, setScore] = useState(0);
  const [selectedOption, setSelectedOption] = useState("");
  const [questions, setQuestions] = useState(questionsQma);
  const [incorrectQuestions, setIncorrectQuestions] = useState([]);

  useEffect(() => {
    const storedIncorrectQuestions = JSON.parse(localStorage.getItem('incorrectQuestionsQma')) || [];
  ...
  const handleAnswer = (option) => {
    ...
    setTimeout(() => {
      ...
      if (currentQuestion < questions.length - 1) {
        ...
        // Quiz complete
        localStorage.setItem('progressQma', Math.min((score + 1) * 10, 100));
        localStorage.setItem('incorrectQuestionsQma', JSON.stringify(incorrectQuestions));
